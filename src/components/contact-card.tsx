...
const ContactCard = ({ title, href, className, icon }: Props) => {
  ...
  // Determine if this is an external link (http, https) - mailto links open in same window
  ...
  return (
    <a
      ...
      className={cn(
        "block w-full h-full p-3 lg:p-5 bg-muted hover:bg-muted-foreground/10 rounded-xl flex justify-between items-center cursor-pointer group transition-all duration-150 ease-[cubic-bezier(0.22, 1, 0.36, 1)]",
        className
      )}
    >
